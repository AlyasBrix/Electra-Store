...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    if ($status == 'Completed' && $old_status != 'Completed') {
        ...
        if (function_exists('sendOrderEmail')) {
            // Clear any output before sending email
            ob_clean();
            ...
            $_SESSION['message'] = "Order status updated to Completed! (Email function not available)";
            $_SESSION['message_type'] = 'success';
        }
    } else {
...
<div class="staff-orders">
    <div class="container">
        ...
        <div class="staff-nav">
            ...
            <a href="/app/electrastore/staff/orders.php" class="nav-link active">
...
@media (max-width: 768px) {
    .staff-orders { padding: 1rem 0; }
    .header-left h1 { font-size: 1.5rem; }
    .stats-orders-grid { grid-template-columns: repeat(2, 1fr); }
    .order-title { flex-direction: column; align-items: flex-start; }
    .order-details-grid { grid-template-columns: 1fr; }
    .status-select-wrapper { flex-direction: column; align-items: stretch; }
    .orders-table { font-size: 0.85rem; }
    .orders-table th, .orders-table td { padding: 0.75rem; }
    .product-info-order { flex-direction: column; text-align: center; }
}
@media (max-width: 480px) {
    .stats-orders-grid { grid-template-columns: 1fr; }
    .info-row { flex-direction: column; }
    .info-label { width: 100%; margin-bottom: 0.25rem; }
    .orders-table thead { display: none; }
    .orders-table tbody tr { display: block; margin-bottom: 1rem; border: 1px solid #e2e8f0; border-radius: 12px; padding: 0.75rem; }
    .orders-table td { display: flex; justify-content: space-between; align-items: center; padding: 0.5rem 0; border: none; }
    .orders-table td::before { content: attr(data-label); font-weight: 600; color: #64748b; font-size: 0.75rem; }
}
</style>
